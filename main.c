...
int main(void) {
	...
	while(fgets(buff, 1024*4, (FILE*)fp)) {
		...
		int index = 0;
		while(ptr != NULL){
			if(index == 0) n->airportId = atoi(ptr);
			else if(index == 1) strcpy(n->IATA, ptr);
			else if(index == 2) strcpy(n->name, ptr);
			else if(index == 3) strcpy(n->airportName, ptr);
			else if(index == 4) n->lat = atof(ptr);
			else if(index == 5) n->lng = atof(ptr);
